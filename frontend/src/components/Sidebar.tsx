  ...
  return (
    <aside className="flex w-full max-w-[280px] flex-col border-r border-[#d8e2d9] bg-[#fcfdf9] p-5">
      <div className="flex items-center gap-3 border-b border-[#e4ebe5] pb-5">
        <div className="grid size-10 place-items-center rounded-xl bg-[#146c52] text-lg font-bold text-white">T</div>
        <div>
          ...
          return (
            <button
              ...
              className={[
                ...
                isActive ? "bg-[#146c52] text-white shadow-sm" : "text-[#17362c] hover:bg-[#edf4f0]",
              ].join(" ")}
            >
              <span className="flex items-center gap-3">
                <span className={isActive ? "text-white" : "text-[#146c52]"}>{item.icon}</span>
                {item.label}
              </span>
      ...
      <div className="mt-auto rounded-2xl border border-[#d8e2d9] bg-[#f3f8f4] p-4">
        <p className="text-xs font-bold uppercase tracking-[.12em] text-[#146c52]">Support</p>
        <p className="mt-2 text-sm text-[#587068]">Need help with clinic operations?</p>
        <button className="mt-3 rounded-xl bg-[#146c52] px-3 py-2 text-sm font-semibold text-white">Contact team</button>
      </div>
    </aside>
  );
}
